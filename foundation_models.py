"""Chronos and PatchTST models used by the extraction scripts."""
...
class PatchTST(nn.Module):
    """Small PatchTST-style transformer with the extraction call signature."""
    ...
    def forward(self, x: torch.Tensor, **kwargs: Any) -> torch.Tensor:
        ...
        return out.reshape(x.shape[0], self.dim, self.horizon)

    @torch.no_grad()
    def representation(self, x: torch.Tensor, *, pool: bool = True, **kwargs: Any) -> torch.Tensor:
        del kwargs
        encoded = self._encode(x)
        if pool:
